...
public class CommonFileAdapter extends BaseFileAdapter {
    ...
    public void initData(RecyclerView.ViewHolder holder, final FileItem fileItem, int position) {
        ...
        switch (holder.getItemViewType()) {
            case TYPE_DOC:
                ImageView docCbChoose = (ImageView) view.findViewById(R.id.cb_choose);
                TextView docName = (TextView) view.findViewById(R.id.tv_name);
                if (mSelectedData.contains(fileItem)) {
                    docCbChoose.setImageDrawable(mContext.getResources().getDrawable(R.mipmap.icon_image_checked));
                } else {
                    docCbChoose.setImageDrawable(mContext.getResources().getDrawable(R.mipmap.icon_image_check));
                }
