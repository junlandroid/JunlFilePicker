...
public abstract class FilePickerBaseActivity extends AppCompatActivity {
    ...
    private static final String TAG = FilePickerBaseActivity.class.getSimpleName();
    ...
    public boolean selectedFilesChange(FileItem fileItem) {
        if (selectedFiles.contains(fileItem)) {
            return selectedFiles.remove(fileItem);
        } else {
            if (selectedFiles.size() < 9) {
                return selectedFiles.add(fileItem);
            } else {
                return false;
            }
        }
